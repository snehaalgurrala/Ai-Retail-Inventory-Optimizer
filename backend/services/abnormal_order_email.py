...
SUBJECT = "🚨 Abnormal Order Investigation Alert"
...
def _executive_summary(inv: dict) -> str:
    return (
        ...
        f"This order exceeds the historical average demand by "
        f"{inv['deviation_pct']:+.0f}% and is significantly above the customer's "
        "normal purchasing pattern. The order may create inventory pressure and "
...
def _badge_html(band: str) -> str:
    ...
    return (
        ...
        f'text-transform:uppercase;letter-spacing:0.6px;">{escape(band)} Risk</span>'
    )
...
def build_abnormal_order_email_html(inv: dict) -> str:
    ...
    header = (
        ...
        '🚨 Abnormal Order Investigation Alert</h1>'
    ...
    investigation = _section("AI Investigation Report", metric_row1 + metric_row2 + pattern_block + explanation)

    # 6) Risk Assessment -----------------------------------------------------
    bg, _ = _BADGE.get(band, _BADGE["Medium"])
    risk_head = (
        '<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>'
        + _metric_tile("Risk Level", band, bg, highlight=True)
        + _metric_tile("Risk Score", f"{score} / 100", bg, highlight=True)
    ...
    footer = (
        ...
        '<div style="opacity:0.7;margin-top:6px;">This alert was automatically generated by '
...
def _build_text_body(inv: dict) -> str:
    math = _inventory_math(inv)
    return "\n".join([
        "Abnormal Order Investigation Alert",
        f"Risk Level: {inv.get('risk_band')} ({int(inv.get('risk_score', 0))}/100)",
        "",
...
def send_abnormal_order_alert_email(inv: dict) -> dict:
    ...
    try:
        ...
        return {
            "success": False,
            "email_sent": False,
            "message": f"Abnormal-order alert could not be sent: {error}",
        }

    return {
        "success": True,
        "email_sent": True,
        "message": f"Abnormal Order Investigation Alert sent to {settings['manager_email']}.",
    }
