...
with kpi_columns[0]:
    render_kpi_card("Total Recommendations", f"{total_recommendations:,}", "Generated by the agent stack", "blue", icon="🤖")
with kpi_columns[1]:
    render_kpi_card(
        "Pending Execution", f"{pending_recommendations:,}", "Awaiting a human decision", "purple",
        icon="⏳", support="Action needed" if pending_recommendations else "All cleared",
    )
with kpi_columns[2]:
    render_kpi_card("High Priority", f"{high_priority_recommendations:,}", "Should be reviewed first", "red", icon="🚩")
